...
forvalues j = 1/`rowsam' {
	if `j' == 1 {
		local s = "0.00"
		local beta_G = sqrt(`h2'+0.5*`n2')
	}
	if `j'==2 {
		local s = "0.25"
		local beta_G = sqrt((`h2'+0.5*`n2'+2*0.0367*sqrt(`h2'*`n2'))/(1+2*0.0367*sqrt(`h2'*`n2')))
	}
	if `j'==3 {
		local s = "0.50"
		local beta_G = sqrt((`h2'+0.5*`n2'+2*0.08*sqrt(`h2'*`n2'))/(1+2*0.08*sqrt(`h2'*`n2')))
	}
	if `j'==4 { 
		local s = "0.75"
		local beta_G = sqrt((`h2'+0.5*`n2'+2*0.132*sqrt(`h2'*`n2'))/(1+2*0.132*sqrt(`h2'*`n2')))
	}
	if `j'==5 {
		local s = "1.00"
		local beta_G = sqrt((`h2'+0.5*`n2'+2*0.185*sqrt(`h2'*`n2'))/(1+2*0.185*sqrt(`h2'*`n2')))
	}
	...
	forvalues k = 1/`rowsPRED' {
		...
		forvalues i = 1/`repl' {
			...
			qui import delimited "DESIGN.2.RHO_AM.`s'.RUN.`i'.HSq.out", clear
			...
			*LOAD DATA
			qui import delimited "DESIGN.2.RHO_AM.`s'.RUN.`i'.pgi", clear
...
coefplot matrix(transpose_meta) matrix(transpose_oriv) matrix(transpose_becker) matrix(transpose_greml), vertical grid(between) ci((2 3)) ylabel(0.3(0.1)0.6) scheme(s1mono) ytitle("Estimated coefficient")  xtitle("Assortative mating") legend(rows(1) lab(2 "Meta-analysis") lab(4 "ORIV") lab(6 "PGI-RC (default)") lab(8 "PGI-RC (GREML unc.)"))
*graph export "C:\Users\41153jvk\Dropbox (Erasmus Universiteit Rotterdam)\GEIGHEI\projects\Measurement error\Figures revision\design2-bf-greml.pdf", replace

local beta_G_wf = sqrt(0.2)
import excel "sim_results.xls", sheet("wf_design2") firstrow clear
replace AM = AM*100
mkmat coef_meta lb_meta ub_meta, matrix(meta) 
mat rownames meta = 0 0.25 0.5 0.75 1
mkmat coef_oriv lb_oriv ub_oriv, matrix(oriv)
...
matrix transpose_meta = meta'
matrix transpose_oriv = oriv'
coefplot matrix(transpose_meta) matrix(transpose_oriv), vertical grid(between) ci((2 3)) scheme(s1mono) ytitle("Estimated coefficient") yline(`beta_G_wf', lpattern(dash)) ylabel(0.3(0.1)0.6) xtitle("Assortative mating") legend(rows(1) lab(2 "Meta-analysis") lab(4 "ORIV"))
graph export "C:\Users\41153jvk\Dropbox (Erasmus Universiteit Rotterdam)\GEIGHEI\projects\Measurement error\Figures revision\design2-wf-greml.pdf", replace
