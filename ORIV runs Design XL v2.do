*DO-FILE TO CHECK THE DATA GENERATED BY RONALD FOR AM DESIGN (4)
*HANS, 18 MARCH 2022, UPDATED WITH BECKER ET AL. AUGUST 2022
...
forvalues j = 1/`rowsam' {
	...
	forvalues k = 1/`rowsPRED' {
		...
		forvalues i = 1/`repl' {
			...
			qui import delimited "DESIGN.4.RHO_AM.`s'.RUN.`i'.HSq.out", clear
			...
			*LOAD DATA
			qui import delimited "DESIGN.4.RHO_AM.`s'.RUN.`i'.pgi", clear
...
mat bf_AM = (bf1 \ bf2)
putexcel set "sim_results.xls", sheet("bf_designXL") modify
putexcel A1=matrix(bf_AM), colnames
mat wf_AM = (wf1 \ wf2)
putexcel set "sim_results.xls", sheet("wf_designXL") modify
putexcel A1=matrix(wf_AM), colnames
mat rmse_AM = (overall_rmse1 \ overall_rmse2)
putexcel set "sim_results.xls", sheet("bf_rmse_designXL") modify
putexcel A1=matrix(rmse_AM), colnames
mat rmse_AM_fe = (overall_rmse1fe \ overall_rmse2fe)
putexcel set "sim_results.xls", sheet("wf_rmse_designXL") modify
putexcel A1=matrix(rmse_AM_fe), colnames

local beta_G = 0.5
local beta_G_wf = sqrt(0.2)
*MAKE PLOTS
import excel "sim_results.xls", sheet("bf_designXL") firstrow clear
replace AM = AM*100
mkmat coef_meta lb_meta ub_meta, matrix(meta) rownames(AM)
mkmat coef_becker lb_becker ub_becker, matrix(becker) rownames(AM)
mkmat coef_oriv lb_oriv ub_oriv, matrix(oriv) rownames(AM)
mkmat coef_becker lb_greml ub_greml, matrix(becker_greml) rownames(AM)
matrix transpose_meta = meta'
matrix transpose_becker = becker'
matrix transpose_oriv = oriv'
matrix transpose_greml = becker_greml'
coefplot matrix(transpose_meta) matrix(transpose_oriv) matrix(transpose_becker) matrix(transpose_greml), vertical grid(between) ci((2 3)) ylabel(0(0.2)0.8) yline(`beta_G', lpattern(dash)) scheme(s1mono) ytitle("Estimated coefficient")  xtitle("AM percentage") legend(rows(1) lab(2 "Meta-analysis") lab(4 "ORIV") lab(6 "PGI-RC (default)") lab(8 "PGI-RC (GREML unc.)"))
graph export "C:\Users\41153jvk\Dropbox (Erasmus Universiteit Rotterdam)\GEIGHEI\projects\Measurement error\Figures revision\designXL-bf-greml.pdf", replace


import excel "sim_results.xls", sheet("wf_designXL") firstrow clear
replace AM = AM*100
mkmat coef_meta lb_meta ub_meta, matrix(meta) rownames(AM)
mkmat coef_oriv lb_oriv ub_oriv, matrix(oriv) rownames(AM)
matrix transpose_meta = meta'
matrix transpose_oriv = oriv'
coefplot matrix(transpose_meta) matrix(transpose_oriv), vertical grid(between) ci((2 3)) scheme(s1mono) ytitle("Estimated coefficient") yline(`beta_G_wf', lpattern(dash)) ylabel(0(0.2)0.8) xtitle("AM percentage") legend(rows(1) lab(2 "Meta-analysis") lab(4 "ORIV"))
graph export "C:\Users\41153jvk\Dropbox (Erasmus Universiteit Rotterdam)\GEIGHEI\projects\Measurement error\Figures revision\designXL-wf.pdf", replace
