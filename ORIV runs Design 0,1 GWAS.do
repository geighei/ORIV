...
*HANS, APRIL 2022, SEPTEMBER 2022
...
mat samplesizesGWAS = (2000 \ 4000 \ 8000 \ 16000 \ 32000)
	...
	forvalues k = 1/`rowsPRED' {
		...
		forvalues i = 1/`repl' {
			...
			*LOAD HERITABILITY ESTIMATES
			qui import delimited "DESIGN.`h'.NGWASPOOLED.`s'.RUN.`i'.HSq.out", clear
...
mat bf_gwas = (bf1 \ bf2 \ bf3  \ bf4 \ bf5)
putexcel set "sim_results.xls", sheet("bf_design_gwas`h'") modify
putexcel A1=matrix(bf_gwas), colnames
mat wf_gwas = (wf1 \ wf2 \ wf3  \ wf4 \ wf5)
putexcel set "sim_results.xls", sheet("wf_design_gwas`h'") modify
putexcel A1=matrix(wf_gwas), colnames
mat rmse_gwas = (overall_rmse1 \ overall_rmse2 \ overall_rmse3 \ overall_rmse4 \ overall_rmse5)
putexcel set "sim_results.xls", sheet("bf_rmse_design_gwas`h'") modify
putexcel A1=matrix(rmse_gwas), colnames
mat rmse_gwas_fe = (overall_rmse1fe \ overall_rmse2fe \ overall_rmse3fe \ overall_rmse4fe \ overall_rmse5fe)
putexcel set "sim_results.xls", sheet("wf_rmse_design_gwas`h'") modify
putexcel A1=matrix(rmse_gwas_fe), colnames
mat F_gwas = (overall_F1 \ overall_F2 \ overall_F3 \ overall_F4 \ overall_F5)
putexcel set "sim_results.xls", sheet("bf_F_design_gwas`h'") modify
putexcel A1=matrix(F_gwas), colnames
mat F_gwas_fe = (overall_Ffe1 \ overall_Ffe2 \ overall_Ffe3 \ overall_Ffe4 \ overall_Ffe5)
putexcel set "sim_results.xls", sheet("wf_F_design_gwas`h'") modify
putexcel A1=matrix(F_gwas_fe), colnames

*MAKE PLOTS
import excel "sim_results.xls", sheet("bf_design_gwas`h'") firstrow clear
drop if _n>5
mkmat coef_meta lb_meta ub_meta, matrix(meta) 
matrix rownames meta = 2.3%(~EA1) 4.2%(~EA2) 7.1% 11.1%(~EA3) 15.4%(~EA4) 
mkmat coef_becker lb_becker ub_becker, matrix(becker) 
matrix rownames becker = 2.3%(~EA1) 4.2%(~EA2) 7.1% 11.1%(~EA3) 15.4%(~EA4) 
mkmat coef_oriv lb_oriv ub_oriv, matrix(oriv) 
matrix rownames oriv = 2.3%(~EA1) 4.2%(~EA2) 7.1% 11.1%(~EA3) 15.4%(~EA4) 
mkmat coef_becker lb_greml ub_greml, matrix(becker_greml) 
matrix rownames becker_greml = 2.3%(~EA1) 4.2%(~EA2) 7.1% 11.1%(~EA3) 15.4%(~EA4) 
matrix transpose_meta = meta'
matrix transpose_becker = becker'
matrix transpose_oriv = oriv'
matrix transpose_greml = becker_greml'
coefplot matrix(transpose_meta) matrix(transpose_oriv) matrix(transpose_becker) matrix(transpose_greml), vertical grid(between) ci((2 3)) scheme(s1mono) ytitle("Estimated coefficient") yline(`beta_G', lpattern(dash)) xtitle("{it:R}-squared of Meta-analysis PGI") ylabel(0(0.25)1) legend(rows(1) lab(2 "Meta-analysis") lab(4 "ORIV") lab(6 "PGI-RC (default)") lab(8 "PGI-RC (GREML unc.)"))
graph export "C:\Users\41153jvk\Dropbox (Erasmus Universiteit Rotterdam)\GEIGHEI\projects\Measurement error\Figures revision\design`h'-bf-gwas-greml.pdf", replace

import excel "sim_results.xls", sheet("wf_design_gwas1") firstrow clear
drop if _n>5
mkmat coef_meta lb_meta ub_meta, matrix(meta) rownames(GWAS)
matrix rownames meta = 2.3%(~EA1) 4.2%(~EA2) 7.1% 11.1%(~EA3) 15.4%(~EA4) 
replace ub_oriv = 1 if ub_oriv > 1
replace lb_oriv = 0 if lb_oriv < 0
mkmat coef_oriv lb_oriv ub_oriv, matrix(oriv) rownames(GWAS)
matrix rownames oriv = 2.3%(~EA1) 4.2%(~EA2) 7.1% 11.1%(~EA3) 15.4%(~EA4) 
matrix transpose_meta = meta'
matrix transpose_oriv = oriv'
coefplot matrix(transpose_meta) matrix(transpose_oriv), vertical grid(between) ci((2 3)) scheme(s1mono) ytitle("Estimated coefficient") ylabel(0(0.25)1) yscale(noextend) yline(`beta_G_wf', lpattern(dash)) xtitle("{it:R}-squared of Meta-analysis PGI") legend(rows(1) lab(2 "Meta-analysis") lab(4 "ORIV"))
graph export "C:\Users\41153jvk\Dropbox (Erasmus Universiteit Rotterdam)\GEIGHEI\projects\Measurement error\Figures revision\design`h'-wf-gwas.pdf", replace
